...
var Moderation = require(__dirname+"\\moderation.js");
...
HitboxChannel.prototype.onmessage = function (message) {
    switch (message.method) {
        ...
        case "userList":
            ...
            break;
        case "notifyMsg":
            this.emit("notify", message.params.join, message.params.part);
            break;
        case "userInfo":
 ...
 */
HitboxChannel.prototype.join = function (notify) {
    if (this.joined) return;
    this.joined = true;
    (notify === true) ? this.send("joinChannel", { notify: true, isAdmin: true }, true) : this.send("joinChannel", { isAdmin: false }, true);
};
